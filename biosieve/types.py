"""Shared type definitions used across reducers and splitters."""
...
class Columns:
    ...
    Args:
        id_col: Column name containing unique sample identifiers.
        BioSieve assumes this column is unique (1 row = 1 id).
        seq_col: Column name containing sequence strings. Required by strategies that operate
        on sequences (e.g., exact deduplication, k-mer redundancy, etc.).
        label_col: Optional classification label column name (e.g., "label").
        Use when applying stratified splits for classification.
        group_col: Optional group identifier column name (e.g., subject_id, taxid).
        Used for group-aware splits and group-aware k-fold.
        cluster_col: Optional cluster identifier column name (e.g., precomputed homology clusters).
        Used for cluster-aware splits, and can be passed to group splitters by setting
        group_col=cluster_col.
        date_col: Optional temporal column name (e.g., collection date, deposition date).
