...
@pytest.fixture
def df_with_duplicates(df_base):
...
def test_happy_path(df_with_duplicates):
...
def test_duplicates_removed(df_with_duplicates):
    reducer = ExactDedupReducer()
    res = reducer.run(df_with_duplicates, COLS)
...
def test_no_duplicates_unchanged(df_base):
...
def test_mapping_schema(df_with_duplicates):
    reducer = ExactDedupReducer()
    res = reducer.run(df_with_duplicates, COLS)
...
def test_no_ids_lost(df_with_duplicates):
    reducer = ExactDedupReducer()
    res = reducer.run(df_with_duplicates, COLS)
    if res.mapping is not None and len(res.mapping) > 0:
