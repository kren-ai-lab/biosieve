...
class TestLoadEmbeddings:
    ...
    def test_ids_align_with_dataframe(
        self, embeddings_files: tuple[Path, Path], df_base: pl.DataFrame
    ) -> None:
        emb_path, ids_path = embeddings_files
        store = load_embeddings(str(emb_path), str(ids_path))
        assert store.ids == df_base["id"].to_list()
    ...
    def test_1d_array_raises(self, tmp_path: Path) -> None:
        ...
        pl.DataFrame({"id": ["a", "b", "c"]}).write_csv(ids_path)
    ...
    def test_row_count_mismatch_raises(self, tmp_path: Path) -> None:
        ...
        ids_path = tmp_path / "ids.csv"
        pl.DataFrame({"id": ["a", "b", "c"]}).write_csv(ids_path)  # only 3 ids for 5 rows
...
class TestInferDescriptorColumns:
    ...
    def test_empty_explicit_cols_raises(self, df_descriptors: pl.DataFrame) -> None:
        with pytest.raises(ValueError, match="empty"):
            infer_descriptor_columns(df_descriptors, explicit_cols=[])


class TestExtractDescriptorMatrix:
    def test_happy_path(self, df_descriptors: pl.DataFrame) -> None:
        ...
        assert mat.X.shape == (df_descriptors.height, len(cols))
        assert mat.X.dtype == np.float32

    def test_non_finite_raises(self, df_descriptors: pl.DataFrame) -> None:
        cols = [c for c in df_descriptors.columns if c.startswith("desc_")]
        bad_df = df_descriptors.with_columns(
            pl.Series(cols[0], [float("nan"), *df_descriptors[cols[0]].to_list()[1:]])
        )
        with pytest.raises(ValueError, match="non-finite"):
            extract_descriptor_matrix(bad_df, cols)

    def test_dtype_preserved(self, df_descriptors: pl.DataFrame) -> None:
        cols = [c for c in df_descriptors.columns if c.startswith("desc_")]
