...
def test_happy_path_precomputed(df_base, cluster_map_file):
...
def test_no_overlap_precomputed(df_base, cluster_map_file):
    splitter = HomologyAwareSplitter(
        mode="precomputed",
        clusters_path=str(cluster_map_file),
        clusters_format="csv",
...
def test_leakage_zero_precomputed(df_base, cluster_map_file):
...
def test_missing_clusters_path_raises(df_base, tmp_path):
    splitter = HomologyAwareSplitter(
        mode="precomputed",
...
@pytest.mark.mmseqs2
def test_happy_path_mmseqs2(df_base, tmp_path):
