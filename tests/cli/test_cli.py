...
def csv_file(tmp_path_factory: pytest.TempPathFactory) -> Path:
...
def test_cli_info_reduce_only() -> None:
    r = _run("info", "--kind", "reduce")
    assert r.returncode == 0
    assert b"exact" in r.stdout


def test_cli_info_split_only() -> None:
...
def test_cli_reduce_exact(csv_file: Path, tmp_path: Path) -> None:
...
def test_cli_reduce_with_map_and_report(csv_file: Path, tmp_path: Path) -> None:
    out = tmp_path / "out.csv"
...
def test_cli_reduce_unknown_strategy_exits_nonzero(csv_file: Path, tmp_path: Path) -> None:
    r = _run(
        "reduce",
        "--input-data",
        str(csv_file),
...
def test_cli_reduce_missing_input_exits_nonzero(tmp_path: Path) -> None:
    r = _run(
        "reduce",
        "--input-data",
...
def test_cli_split_random(csv_file: Path, tmp_path: Path) -> None:
...
def test_cli_split_kfold(csv_file: Path, tmp_path: Path) -> None:
    outdir = tmp_path / "splits"
    r = _run(
        "split",
        "--input-data",
...
def test_cli_split_unknown_strategy_exits_nonzero(csv_file: Path, tmp_path: Path) -> None:
    r = _run(
        "split",
        "--input-data",
        str(csv_file),
...
def test_cli_validate_basic(csv_file: Path) -> None:
...
def test_cli_validate_missing_input_exits_nonzero(tmp_path: Path) -> None:
